<template>
        ...
        <div class="text-input">
          ...
          <div class="upload-photo">
            ...
            <div class="buttons">
              ...
              <button class="preview" @click.prevent="openPreview">
                Preview Photo
              </button>
            </div>
  ...
  <dialog ref="preview" class="dialog" :class="{ closeModal: closeModal }">
    <div class="image-container">
      <img :src="[photochange ? newImage : imageURL]" />
    </div>
    <i @click="closeDialog" class="fa-solid fa-xmark"></i>
  </dialog>
  <dialog ref="success" class="dialog" :class="{ closeModal: closeModal }">
    <div class="modal-content">
      <p>Successfully Edited the Recipe</p>
      <router-link to="/myrecipes" class="create">Go Back</router-link>
    </div>
    <i @click="closeDialog" class="fa-solid fa-xmark"></i>
  </dialog>
...
export default {
  ...
  data() {
    return {
      notUploaded: true,
      modalActive: false,
      modalMessage: "Successfully Edited the Recipe",
      changephoto: false,
      closeModal: false,
      fileName: "",
      photochange: false,
  ...
  methods: {
    ...
    async createRecipe() {
      if (
        ...
        this.steps[0].text !== undefined &&
        this.ingredients[0].name !== undefined
      ) {
        this.error = false;
        this.errorMsg = "";
        this.loading = true;
  ...
  computed: {
    ...
    recipeID() {
      return this.$store.state.recipeID;
    },
    title: {
      get() {
      ...
      set(payload) {
        this.$store.commit("setRecipeTitle", payload);
      },
    },
    description: {
      get() {
        return this.$store.state.recipeDescription;
      },
      set(payload) {
        this.$store.commit("setRecipeDescription", payload);
      },
    },
    servings: {
      get() {
        return this.$store.state.recipeServings;
      },
      set(payload) {
        this.$store.commit("setRecipeServings", payload);
      },
    },
    steps: {
      get() {
        return this.$store.state.recipePreparation;
      },
      set(payload) {
        this.$store.commit("setRecipeDescription", payload);
      },
    ...
    imageURL() {
      ...
      return this.$store.state.recipePhotoURL;
    },
    ingredients: {
      get() {
      ...
      set(payload) {
        return this.$store.commit("setRecipeIngredients", payload);
      },
    },
    youTubeURL: {
      get() {
        return this.$store.state.recipeYoutubeLink;
      },
      set(payload) {
        this.previewVideo = false;
        return this.$store.commit("setRecipeYoutubeLink", payload);
      },
    },
    time: {
      get() {
        return this.$store.state.recipePreparationTime;
      },
      set(payload) {
        return this.$store.commit("setRecipePreparationTime", payload);
      },
    },
  },
};
